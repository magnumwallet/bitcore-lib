...
PrivateKey._transformBuffer = function(buf, network) {
  ...
  if(!info.network || (network && info.network !== Networks.get(network)))  {
    ...
    buf = Buffer.concat([new Buffer([Networks.get(network).privatekey]),
      buf.slice(1, 29),
      Hash.sha256sha256(buf.slice(1, 29)).slice(0,4)],
      buf.length === 1 + 32 + 1 && buf[1 + 32 + 1 - 1] === 1 ? [1] : []);
      console.log("Debug: ", buf);

  }
  console.log("Debug: ", info.network);
